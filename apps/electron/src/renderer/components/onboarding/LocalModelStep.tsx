...
export function LocalModelStep({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmedUrl) {
      setModelError(t('onboarding.localModel.errorUrlRequired'))
      return
    }
    if (parsedModels.length === 0) {
      setModelError(t('onboarding.localModel.errorModelRequired'))
      return
    }
  ...
  return (
    <StepFormLayout
      ...
      actions={
        <>
          <BackButton onClick={onBack} disabled={isDisabled}>{t('onboarding.back')}</BackButton>
          <ContinueButton
            type="submit"
      ...
      <form id="local-model-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          <Label htmlFor="local-model">
            ...
            <span className="text-foreground/30">{t('onboarding.localModel.modelRequired')}</span>
          </Label>
          <div className={cn(
            "rounded-md shadow-minimal transition-colors",
            "bg-foreground-2 focus-within:bg-background",
          ...
          <p className="text-xs text-foreground/30">
            {t('onboarding.localModel.modelHintPrefix')} <code className="text-foreground/40">ollama pull</code>{t('onboarding.localModel.modelHintSuffix')}
          </p>
        </div>
